...
pub fn check_database(
    ...
    let bad_paths: Vec<String> = {
        let mut stmt = conn.prepare_cached("SELECT file_path FROM files").expect("SQL statement preparing failed: ");
        let rows: Vec<Result<String, _>> = stmt.query_map([], |row| row.get::<_, String>(0)).expect("Getting Rows failed: ").collect();
    ...
    if !bad_paths.is_empty() {
        let tx = conn.transaction().expect("Transaction creation failed");
        {
            ...
            let mut stmt = tx.prepare_cached(&query).expect("SQL statement preparing failed: ");
            stmt.execute(rusqlite::params_from_iter(bad_paths.iter())).expect("SQL statement preparing failed: ");
        }
...
pub fn initialize_database(
    ...
    pooled_connection.execute("CREATE INDEX IF NOT EXISTS idx_file_path ON files (file_path)", []).expect("Indexing failed: ");
}
